...
def list_sum(x: List, index: int = 0) -> Any:
    # 基本情况，如果已经到达列表末尾，则返回0
    if index == len(x):
        return 0
    # 返回当前元素和剩余列表的和
    return x[index] + list_sum(x, index + 1)
...
class EfficientViTBackbone(nn.Module):
    def __init__(self, width_list: List[int], depth_list: List[int], in_channels=3, dim=32, expand_ratio=4, norm="bn2d",
        ...
        for i in range(3, max(len(width_list), len(depth_list))):
                # 我们检查当前索引是否在每个列表的长度范围内
            w = width_list[i] if i < len(width_list) else None
            d = depth_list[i] if i < len(depth_list) else None
            stage = []
            block = self.build_local_block(
                in_channels=in_channels,
                out_channels=w,
