...
class LogicalLoss(nn.Module):
    def __init__(self, hyp, model, distiller='l2') -> None:
        ...
        elif distiller in ['CTtargetloss']:
            self.logical_loss = CTtargetloss(hyp, model)
            
        elif distiller in ['CloserT']:
            self.logical_loss = CloserT(hyp, model)
...
class CTtargetloss(nn.Module):
    def __init__(self, hyp, model):
        ...
        self.anchors = model.model[-1].anchors
        self.stride = model.model[-1].stride
        self.objloss = torch.nn.L1Loss(reduction='none')
        self.cls_loss = torch.nn.L1Loss(reduction='none')
    ...
    def forward(self, s_p, t_p):
        ...
        for i in range(len(s_p)):
            ...
            iou = bbox_iou(s_pbox, t_pbox, WIoU=True) # iou(prediction, target)
            # lbox += (1.0 - iou).mean()  # iou loss
            ...
            t_obj_scale = t_pobj.sigmoid()
            b_obj_scale = t_obj_scale.repeat(1, 1, 1, 1, 4)
            lobj += torch.mean(self.objloss(s_pobj, t_pobj) * b_obj_scale)
...
class CloserT(nn.Module):
    def __init__(self, hyp, model):
        ...
        # 定义alpha和beta为可训练参数
        self.alpha = nn.Parameter(torch.tensor(1.0))  # 初始值可以根据需要调整
    ...
    def forward(self, s_p, t_p):
        ...
        for i in range(len(s_p)):
            ...
            t_obj_scale = t_pobj.sigmoid()
            b_obj_scale = t_obj_scale.repeat(1, 1, 1, 1, 4)
            lobj += torch.mean((self.alpha * self.L1(s_pobj, t_pobj) + beta * self.BCE(s_pobj, t_pobj)) * b_obj_scale)

            if cls_num > 1:
                c_obj_scale = t_obj_scale.repeat(1, 1, 1, 1, cls_num)
                lcls += torch.mean((self.alpha * self.L1(s_pcls, t_pcls) + beta * self.BCE(s_pcls, t_pcls)) * c_obj_scale)
